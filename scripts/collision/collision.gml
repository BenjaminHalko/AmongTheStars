function Collision(_x,_y) {
	...
	for(var i = 0; i < array_length(global.collisionMaps); i++) {
		if(tilemap_get_at_pixel(global.collisionMaps[i],ceil(bbox_left-x+_x),ceil(bbox_top-y+_y)) or
	...
	tilemap_get_at_pixel(global.collisionMaps[i],ceil(bbox_right-x+_x-1),ceil(bbox_bottom-y+_y-1))) return true;	
	}
